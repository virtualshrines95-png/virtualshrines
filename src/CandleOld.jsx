...
function Candle({cname}) {
	...
	const reactivate = async () => {
		const { data, error } = await supabase.from('Candles')
			.select('wasActive')
			.eq('id',cname)
			.single();
		   if (error) console.error('Error fetching candle time', error);
		   else {
			setWasActive(data.wasActive);}

		  };
		  
	const updateCandleStatus = async (id, newTimeStamp) => {
		    ...
		    const { data, error } = await supabase
		      .from('Candles') // Replace 'todos' with your table name
		      .update({ wasActive: newTimeStamp }) // Object with the column(s) to update and their new value(s)
		      .eq('id', id) // Filter the row where the 'id' column matches the provided ID
	...
	const increaseCandleCount = async () => {
		const { data, error } = await supabase.from('Candles')
		   ...
		   if (error) console.error('Error fetching candle times lit', error);
			...
			console.log(data.timesLit);
		}
		let times = data.timesLit+1;

		await supabase.from('Candles') 
		  .update({ timesLit: times}) // Object with the column(s) to update and their new value(s)
		  .eq('id', cname) // Filter the row where the 'id' column matches the provided ID
		  .select(); // Optional: use select() to return the updated data (recommended in v2 of the JS SDK)
	...
	const handleClick = (timeRemaining=BURN_TIME) => {
		...
		setIsActive(true);
		if (wasActive==0) {
			updateCandleStatus(cname, (new Date()).getTime());
